...
class HierNet(torch.nn.Module):
    def __init__(self, in_channels, out_channels, num_layers,num_relations,use_graph, hls_dim, drop_out=0.0, pool_ratio=0.5):
        ...
        self.use_graph = use_graph
        self.graph_weight = torch.nn.Parameter(torch.tensor(0.3))
        ...
        self.trans_conv = TransConv(in_channels, out_channels, num_layers=1, num_relations=num_relations,
                                    dropout=drop_out)
...
if __name__ == "__main__":
    ...
    for fold, (train_idx, test_idx) in enumerate(kfold.split(dataset_list)):
        ...
        model = HierNet(in_channels=data_ini.num_features, use_graph=True, out_channels=128, num_layers=3,
                        hls_dim=1, drop_out=0.0, num_relations=3)
        ...
        for epoch in range(500):
            ...
            if train_mape < min_train_mape:
                ...
                }, os.path.join(model_dir,f'{target[tar_idx]}_fold{fold + 1}_sgformer_rgcn+gf_5z_mae_h64_checkpoint_train.pt'))
            ...
            if test_mape < min_test_mape:
                ...
                }, os.path.join(model_dir, f'{target[tar_idx]}_fold{fold + 1}_sgformer_rgcn+gf_5z_mae_h64_checkpoint_test.pt'))
