...
if __name__ == "__main__":
    ...
    dataset_list = generate_dataset(dataset_dir, dataset, print_info=True) #False
    ...
    for fold, (train_idx, test_idx) in enumerate(kfold.split(dataset_list)):
        ...
        model = HierNet(in_channels=data_ini.num_features, hidden_channels=64, num_layers=3, conv_type='pna',
                        hls_dim=1, deg=deg,drop_out=0.0)
        ...
        for epoch in range(500):
            ...
            if (train_mape < test_mape) or (abs(train_mape - test_mape) <= tolerance):
                if train_mape < min_train_mape:
                    ...
                    }, os.path.join(model_dir, f'{target[tar_idx]}_fold{fold + 1}_pna+gf_5z_mape_h64_checkpoint_train.pt'))
                ...
                if test_mape < min_test_mape:
                    ...
                    }, os.path.join(model_dir, f'{target[tar_idx]}_fold{fold + 1}_pna+gf_5z_mape_h64_checkpoint_test.pt'))
