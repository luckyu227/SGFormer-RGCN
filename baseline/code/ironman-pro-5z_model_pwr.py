...
def test(model, loader, epoch):
    ...
    with torch.no_grad():
        ...
        if epoch % 10 == 0:
            print('pred.y:', out / 10)
            print('data.y:', true_y[tar_idx] / 10)
        ds = loader.dataset
        mse = mse / len(ds)
        mape = mape / len(ds)
    return mse, mape
    ...
    for fold, (train_idx, test_idx) in enumerate(kfold.split(dataset_list)):
        ...
        for epoch in range(500):
            ...
            if train_mape < min_train_mape:
                ...
                }, os.path.join(model_dir, f'{target[tar_idx]}_fold{fold + 1}_ironman-pro_5z_mae_h64_checkpoint_train.pt'))
            ...
            if test_mape < min_test_mape:
                ...
                }, os.path.join(model_dir, f'{target[tar_idx]}_fold{fold + 1}_ironman-pro_5z_mae_h64_checkpoint_test.pt'))
